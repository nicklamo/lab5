...
function loadStatsPage(){
	...
	{
		var homeScore = parseInt(table.rows.item(i).cells.item(2).innerHTML);
		var awayScore = parseInt(table.rows.item(i).cells.item(3).innerHTML);
		if(homeScore > awayScore)
		{
			//make the win count +1
			wins +=1
			table.rows.item(i).cells.item(4).innerHTML = "Win"
		}
		else
		{
			//make the loss count +1
			losses += 1
			table.rows.item(i).cells.item(4).innerHTML = "Loss"
		}
...
function loadPlayersPage()
{
	for(i = 0; i < players.length; i++ ){
		...
		var node = document.createTextNode(players[i].name);
...
//worked on with Bryce Woods, Trevor Hunter, and Marc Laroc
